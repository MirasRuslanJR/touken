...
# Запрос подсказки у внешней модели. Наружу уходят только обезличенные
# показатели (см. app/assist.py), но сам факт обращения по конкретному
# ребёнку журналируется: проверяющий должен видеть, что данные покидали
# контур, пусть и без имён.
AI_ASSIST = "ai_assist"
...
ACTION_TITLES = {
    ...
    AI_ASSIST: "Запросил ИИ-подсказку (без персональных данных)",
}
