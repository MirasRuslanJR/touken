...
def student_card(sid: int, request: Request, user: Psychologist = Depends(require_casework), db: Session = Depends(get_db)):
    ...
    # странице опроса обещано, что психологу это не показывается. Обещание
    # касается интерфейса — в таблице choices связь остаётся, потому что без
    # неё не посчитать явку и не построить динамику; её ограничивают журнал
    # доступа и срок хранения (app/retention.py).
    dynamics = []
    for sv in surveys:
        if sv.id in purged:
